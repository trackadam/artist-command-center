...
return [
createMetric("Account", profileRecord ? `${stringifyCell(profileRecord.first_name)} ${stringifyCell(profileRecord.last_name)}` : "Not loaded", profileRecord?.email ? stringifyCell(profileRecord.email) : undefined),
...
return ( <details className="distribution-json-details"> <summary>View raw JSON</summary> <pre>{JSON.stringify(data, null, 2)}</pre> </details>
);
}
...
return ( <div className="distribution-simple-list">
...
return ( <div className="distribution-table-wrap"> <table className="distribution-table"> <thead> <tr>
...
{columns.map((column) => ( <td key={column}>{stringifyCell(row[column])}</td>
))} </tr>
))} </tbody> </table>
{recordRows.length > 20 ? <p className="distribution-table-note">Showing first 20 rows.</p> : null} </div>
);
}
...
return ( <article className="distribution-panel-card"> <div className="asset-card-header distribution-mini-header"> <div> <span className="asset-type-pill">{endpoint.path}</span> <h3>{endpoint.label}</h3> <p>{endpoint.description}</p> </div>
<button className="mini-action-btn" type="button" onClick={() => onLoad(endpoint)} disabled={state.loading || !connection}>
{state.loading ? "Loading..." : state.data ? "Refresh" : "Load"} </button> </div>
...
return ( <section className="page-section distribution-page distribution-dashboard-page"> <div className="section-header distribution-hero-header"> <div> <p className="eyebrow">Too Lost Integration</p> <h2>Distribution Dashboard</h2> <p>
Manage Too Lost sandbox connection, catalog pulls, analytics snapshots, sales/royalty checks, and lookup data from inside Track Adam OS. </p> </div> <div className="distribution-hero-actions">
{!connected ? (
<button className="primary-btn" type="button" onClick={handleConnect} disabled={actionLoading || Boolean(configError)}>
{actionLoading ? "Opening Too Lost..." : "Connect Too Lost Sandbox"} </button>
) : (
<>
<button className="primary-btn" type="button" onClick={loadDashboardBasics} disabled={actionLoading || expired}>
{actionLoading ? "Loading..." : "Load Dashboard"} </button> <button className="secondary-btn" type="button" onClick={handleDisconnect} disabled={actionLoading}>
Disconnect </button>
</>
)} </div> </div>
